...
| `OTEL_DOCKER_GROUP_ID` | Le GID du groupe `docker` sur la machine hôte. (Essentiel pour lire le socket Docker). | *À récupérer avec `cat /etc/group \| grep docker`* |
| `HOSTNAME` | Nom de la machine hôte utilisé pour identifier la provenance des métriques. | `mon-serveur-dev` |
